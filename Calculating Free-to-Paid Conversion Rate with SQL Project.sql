...
SELECT
ROUND(COUNT(first_date_purchased) * 100.0 / COUNT(first_date_watched), 2) AS conversion_rate,
ROUND(SUM(days_diff_reg_watch) / COUNT(days_diff_reg_watch), 2) AS avg_reg_watch,
ROUND(SUM(days_diff_watch_purch) / COUNT(days_diff_watch_purch), 2) AS avg_watch_purch
FROM (
	SELECT 
		e.student_id,
		i.date_registered,
